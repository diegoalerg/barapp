<?php
header("Content-type: application/pdf");              //ME ABRE EL DOCUMENTO QUE LE MANDE LA RUTA
header("Content-Disposition: inline; filename=documento.pdf");
readfile('/opt/bitnami/apache/htdocs/registros/'.$data["id"]); //COLOCAR DEPENDIENDO DEL NOMBRE PARA VISUALIZAR DESDE EL HISTORIAL
                                                                                //DEBO HACER QUE LA FECHA COINCIDA PARA ENCONTRAR EL ARCHIVO
                                                                                    //LA FECHA NO ES LA MEJOR OPCION, CREAR ID?CONTADOR INFINITO?

                                                                                    //VARIABLE QUE MANDO DESDE EL CONTROLADOR? CONTADOR?
/*
if (isset($data)) {
   // global $data;
    //var_dump($data);
   // var_dump($data["items"][0]->item);
    ...
    $pdf->Output();
    //$pdf->Output('/opt/lampp/htdocs/ejercicioscipsa/BarTool/registros/test/'.$data["fecha"].'.pdf','F');
} else {
    echo 'No hay info';
}
